...
from src.utils.load_yaml import DNI_EMAIL,DNI_PASSWORD,MAX_SLEEP, SCREENSHOTS_DIR
...
class LoginPage:
    ...
    async def login(self):
        ...
        dni_logger.debug("Login Page Loaded")

        # Enter email and password
        await self.page.fill('input[name="Input.Email"]', DNI_EMAIL)
        await asyncio.sleep(1)
        
        dni_logger.debug("Email Filled ")
        
        await self.page.fill('input[name="Input.Password"]', DNI_PASSWORD)
        
        dni_logger.debug("Password Filled ")
        ...
        dni_logger.debug("Captcha Solved ")
        ...
        if await self.page.locator('//*[@id="sidebar"]').is_visible():
            dni_logger.debug("Logged in successfully")
            return True
        
        else:
            dni_logger.error("An error occurred during the login process")

            screenshot_path = f"{SCREENSHOTS_DIR}/login_error.png"
            await self.page.screenshot(path=screenshot_path)
            print(f"Screenshot saved at {screenshot_path}")
            
            insurance_name = "DUBAI NATIONAL INSURANCE"

...
class Category1Page:
    ...
    #     try:
    #         selector = self.get_field_selector(field_name, column) if use_dynamic_selector else field_name
    #         takaful_logger.debug(f"{field_name} Value for Column {column}: (Before Apply) {value}")
    #         await self.page.select_option(selector, value=value.rstrip(), timeout=3000)
    #         takaful_logger.debug(f"{field_name} Value for Column {column}: (After Apply) {value}")
    #         await asyncio.sleep(MAX_SLEEP)
    #         takaful_logger.debug(f"{field_name} Column {column} Filled")
    #         return True
    #     except Exception as e:
    ...
    async def fill_category_1(self, catA, region, tpa, network):
        takaful_logger.debug("Category A Started")
        print('Cat A')
        
        try:
            await asyncio.sleep(10)
            await screenshot_and_compare(self.page, "takaful", "TAKAFUL_EMARAT_3")
        except Exception as e:
        ...
        try:
            ...
            await asyncio.sleep(MAX_SLEEP)
            await asyncio.sleep(MAX_SLEEP)
            takaful_logger.debug("Checkbox Marked")
        except Exception as e:
            takaful_logger.error(f"An error occurred while marking the checkbox: {e}")
            return False
        ...
        if catA['TPA'].iloc[0] == 'Aafiya':
            fields = [
                ...
                {"name": "Geographical Area of Cover", "value": str(catA['Territory'].iloc[0]), "column": column},
                {"name": "Room Category", "value": str(catA['Room Category'].iloc[0]), "column": column},
                ...
                {"name": "OP Services - Lab, Radiology, etc", "value": str(catA['Op Co Insurance'].iloc[0]), "column": column},
                {"name": "Physiotherapy treatment, subject to prior approval", "value": str(catA['Physio'].iloc[0]), "column": column},
                ...
                {"name": "Maternity OP Services", "value": str(catA['Maternity - Married Females CO'].iloc[0]), "column": column},
                ...
                {"name": "Alternative Medicine", "value": str(catA['Alternative Medicine'].iloc[0]), "column": column},
                ...
                {"name": "Air Ticket (Referral from Insurance Company)", "value": str(catA['Airfare for IP Treatment'].iloc[0]), "column": column},
                {"name": "Shingrix Vaccine", "value": str(catA['Shiqinx Vaccine'].iloc[0]), "column": column}, 
        ...
        takaful_logger.debug("Category A Completed")

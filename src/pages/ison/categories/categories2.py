...
class Categories2:
    ...
    def __init__(self, page, df2, cat2=None):
        self.page = page
        self.df2 = df2
        # cat2 parameter is kept for backward compatibility

    def get_value(self, column_name):
        """Fetch value from specified column in the DataFrame."""
        try:
            # Fetch value from the specific column and convert to string
            # Note: Using index 1 for Category B (versus index 0 for Category A)
    ...
    async def _apply_selection(self, field):
        ...
        try:
            tpa = str(self.df2['TPA'].values[0]).strip()
            region = str(self.df2['Region'].values[0]).strip()
            network = str(self.df2['Network'].values[0]).strip()
            ...
            # SMART EXTRACTION: Only handle loading if needed
            if field_name != "TPA":
    ...
    async def categories2_information(self):
        ...
        ison_logger.info("Filling Category B Information")

        # Screenshot before filling fields
        try:
            await asyncio.sleep(10)
            await screenshot_and_compare(self.page, "ison", "ISON_4")  # Updated screenshot name for Category B
        except Exception as e:
        ...
        for field in self.FIELDS:
            field_name = field["name"]
            try:
                if await self._apply_selection(field):
                    successful_fields += 1

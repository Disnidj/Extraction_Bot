...
class Category1PageNAS:
    ...
    async def handle_optical_benefit(self, value, column):
        try:
            ...
            if len(optical_selectors) <= 1:
                qatar_logger.warning(f"⚠️ Optical Benefit: Only {len(optical_selectors)} selector(s) found, skipping field")
                return False
    ...
    async def fill_category_1(self, catA, region, tpa, network, portal_name):
        qatar_logger.debug("Category A Started")
        print('Cat A')
        
        try:
            await asyncio.sleep(10)
            await screenshot_and_compare(self.page, "qatar", "QATAR_3")
            await self.page.get_by_role("checkbox").check()
            await asyncio.sleep(MAX_SLEEP)
            # await asyncio.sleep(MAX_SLEEP)
            qatar_logger.debug("Checkbox Marked")
        except Exception as e:
            qatar_logger.error(f"An error occurred while marking the checkbox: {e}")
        ...
        fields = [
            ...
            # {"name": "Network", "value": str(catA['Network'].iloc[0]), "column": column},  #need to confirm
            {"name": "Territorial Scope of Coverage", "value": str(catA['Territory'].iloc[0]), "column": column},
            {"name": "IP Coinsurance", "value": str(catA['IP Coinsurance'].iloc[0]), "column": column},
            {"name": "Room type", "value": str(catA['Room Category'].iloc[0]), "column": column},
            {"name": "OP Consultation", "value": str(catA['Deductable'].iloc[0]), "column": column},
            {"name": "OP Services Co-pay", "value": str(catA['Diagnostic Copay'].iloc[0]), "column": column},
            {"name": "Pharmacy Limit", "value": str(catA['Pharmacy Sublimit'].iloc[0]), "column": column},
            ...
            {"name": "Laboratory , Radiology , Pathology and Diagnostic services.", "value": str(catA['Lab & X-Ray'].iloc[0]), "column": column},
            {"name": "X-ray, MRI, CT-scan, Ultra Sound and Endoscopy diagnostic services", "value": str(catA['Diagnostic/Lab Copay'].iloc[0]), "column": column},
            {"name": "Organ Transplantation", "value": str(catA['Organ Transplant'].iloc[0]), "column": column},
            {"name": "New Born Cover", "value": str(catA['New born cover'].iloc[0]), "column": column},
            ...
            {"name": "Repatriation of Mortal Remains to the Country of Domicile", "value": str(catA['Repatriation of Mortal remains'].iloc[0]), "column": column},
            {"name": "Alternative Medicine", "value": str(catA['Alternative Medicine'].iloc[0]), "column": column},  #need to confirm
            {"name": "Psychiatric Benefits", "value": str(catA['Psychiatry'].iloc[0]), "column": column},
            {"name": "Maternity Benefit", "value": str(catA['Maternity In-patient Sublimit'].iloc[0]), "column": column},
            {"name": "Maternity Coinsurance", "value": str(catA['Maternity In-patient Co-pay'].iloc[0]), "column": column},
        ...
        qatar_logger.debug("Category A Completed")

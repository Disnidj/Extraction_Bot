...
class Category1Page:
    ...
    async def select_option_with_error_handling(self,region, tpa, network, field_name, value, column, use_dynamic_selector=True):
        try:
            ...
            await extract_dropdown_values(self.page, region, tpa, network, field_name, selector, 'Medgulf')
    ...
    async def fill_category_1(self, catA,region, tpa, network):
        medgulf_logger.debug("Category A Started")
        print('Cat A')
        
        try:
            try:
                await asyncio.sleep(10)
                await screenshot_and_compare(self.page, "medgulf", "MEDGULF_3")
            except Exception as e:
        ...
        # Use column 1 for both Nextcare and non-Nextcare
        column = 1
        if catA['TPA'].iloc[0] == 'Nextcare':
            fields = [
                ...
                # {"name": "Medical Network", "value": str(catA['Network'].iloc[0]).strip(), "column": column},
                {"name": "In-patient Room Type", "value": str(catA['Room Category'].iloc[0]), "column": column},
                {"name": "Deductible per Consultation", "value": str(catA['Op Co Insurance'].iloc[0]), "column": column},
                {"name": "Prescribed Drugs & Medicines Annual Limit", "value": str(catA['Alternative Medicine'].iloc[0]), "column": column},
                {"name": "Prescribed Drugs & Medicines Co-pay", "value": str(catA['Alternative Medicine Co'].iloc[0]), "column": column},#
                {"name": "Diagnostics Co-pay", "value": str(catA['Lab & Diagnostics Co-pay'].iloc[0]), "column": column},
            ...
            # For non-Nextcare TPA
            fields = [
                # {"name": "Network", "value": str(catA['Network'].iloc[0]), "column": column},
                {"name": "Annual Medical Limit", "value": str(catA['Annual Limit'].iloc[0]), "column": column},#
                {"name": "Territories covered", "value": str(catA['Territory'].iloc[0]), "column": column},
                {"name": "IP & Day Care Co-pay", "value": str(catA['Diagnostic Copay'].iloc[0]), "column": column},
                {"name": "In-patient Room Type", "value": str(catA['Room Category'].iloc[0]), "column": column},
                {"name": "OP Consultation Co-pay", "value": str(catA['Op Co Insurance'].iloc[0]), "column": column},
                {"name": "OP Diagnostics Co-pay", "value": str(catA['Diagnostic/Lab Copay'].iloc[0]), "column": column},
                ...
                {"name": "Prescribed Medicine Type", "value": str(catA['Medicine'].iloc[0]), "column": column},
                {"name": "Physiotherapy", "value": str(catA['Physio'].iloc[0]), "column": column},
                {"name": "Physiotherapy Co-pay", "value": str(catA['Physio Co'].iloc[0]), "column": column},
                {"name": "Nursing at home", "value": str(catA['Home Nursing Charges'].iloc[0]), "column": column},
                {"name": "Repatriation of Mortal Remains", "value": str(catA['Repatriation of Mortal remains'].iloc[0]), "column": column},
                {"name": "Alternative Medicine Limit", "value": str(catA['Alternative Medicine'].iloc[0]), "column": column},
                {"name": "Alternative Medicine Co-pay", "value": str(catA['Alternative Medicine Co'].iloc[0]), "column": column},#
                {"name": "OP Psychiatric Benefits Limit", "value": str(catA['Renal Dialysis'].iloc[0]), "column": column}, 
                {"name": "OP Psychiatric Co-pay", "value": str(catA['Psychiatry'].iloc[0]), "column": column},
                {"name": "Maternity benefit limit", "value": str(catA['Maternity - Married Females'].iloc[0]), "column": column},
                {"name": "Maternity benefit co-pay", "value": str(catA['Maternity - Married Females CO'].iloc[0]), "column": column},
                ...
                {"name": "Routine Health Check-up", "value": str(catA['Telehealth Consultation'].iloc[0]), "column": column},
            ]
        ...
        medgulf_logger.debug("Category A Completed")

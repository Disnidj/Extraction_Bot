import asyncio
import os
# from src.utils.load_yaml import RAK_QUOTATION_DIR
    ...
    async def download_quotation(self, quotation_no ,unique_categories_list,df2):
        try:
            ...
            try:
                ...
                rak_logger.error(f"Proceed button click failed: {e}")
            ...
            rak_logger.debug("Proceed Button Clicked")
            await self.page.wait_for_load_state('networkidle')

            button = self.page.get_by_role("button", name="Proceed to Submission ")
            try:
                await button.wait_for(state="visible", timeout=10000)
                await button.click()
            except TimeoutError:
                raise Exception("Button 'Proceed to Submission' is not visible within 10 second.")

            # await self.page.get_by_role("button", name="Proceed to Submission ").click()
            await asyncio.sleep(1)
            
            rak_logger.debug("Proceed to Submission Button Clicked")
            ...
            for index, (category, locator) in enumerate(available_categories):
                ...
                # Extract dropdown values for broker commission
                field_name = "Broker Commission"
                await extract_dropdown_values(self.page, '', '', '', field_name, locator, 'RAK Insurance')
            ...
            #     rak_logger.info(f"Quotation No: {quotation_no} send to refferal")
            ...
            # else:
                ...
                #         await self.page.reload()
                #         await self.page.wait_for_load_state('networkidle')
                #         rak_logger.debug("Page Reloaded for Quotation Status Check")
                #         await asyncio.sleep(1)
                ...
                #             rak_logger.error(f"Quotation No: {quotation_no} is REJECTED By RAK please check data...")
                #             raise Exception(f"Quotation No: {quotation_no} is REJECTED By RAK please check data...")
                        
                # # Perform tasks using the dynamic Quotation No
                # await self.page.get_by_role("gridcell", name=quotation_no).click()
                # await asyncio.sleep(1)

                # rak_logger.debug("Quotation No Clicked")
            ...
            rak_logger.error('Error in Download Page')
            return False
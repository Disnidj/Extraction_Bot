...
class OrientAuraAPIExtractor:
    ...
    async def extract_all_benefits(self):
        ...
        async with OrientAuraAPIClient(self.auth) as client:
            ...
            # Level 0: Use hardcoded Group (Nextcare Sme - 173)
            print("\n📥 Level 0: Using hardcoded Group: Nextcare Sme (ID: 173)")
            orient_aura_logger.info("Level 0: Using hardcoded Group - Nextcare Sme")
            
            # Get hardcoded group configuration
            hardcoded_group = ORIENT_MAPPING["group"]
            group_id = hardcoded_group["group_id"]
            group_name = hardcoded_group["group_name"]
            reinsurer_company_id = hardcoded_group["reinsurer_company_id"]
            
            print(f"   ✓ Using Group: {group_name} (ID: {group_id})")
            orient_aura_logger.info(f"Using Group: {group_name} (ID: {group_id})")
            
            # Process the hardcoded group only

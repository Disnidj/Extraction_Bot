...
class Category3Page:
    ...
    def get_field_selector(self, field_name, column=3):
    ...
    async def fill_category_3(self, catC, region, tpa, network):
        fidelity_logger.debug("Category C Started")
        print('Category C Started')

        try:
            await asyncio.sleep(10)
            await screenshot_and_compare(self.page, "fidelity", "FIDELITY_5")
        except Exception as e:
        ...
        if str(catC['Plan Selection'].iloc[0]).strip() == "NextcareEmployees Guard - Plan 1 - 5":
            fields = [
                ...
                {"name": "Medical Network", "value": str(catC['Pharmacy Sublimit'].iloc[0]), "column": column},
                {"name": "Room type", "value": str(catC['Room Category'].iloc[0]), "column": column},
                {"name": "Deductible per Consultation", "value": str(catC['Deductable'].iloc[0]), "column": column},
                {"name": "Prescribed Drugs & Medicines", "value": str(catC['Limit of Phamacy'].iloc[0]), "column": column},
                {"name": "Pharmacy Co-pay", "value": str(catC['Pharmacy'].iloc[0]), "column": column},
                {"name": "Diagnostics Co-pay", "value": str(catC['Diagnostic/Lab Copay'].iloc[0]), "column": column},
            ...
            fields = [
                # {"name": "Network", "value": str(catC['Pharmacy Sublimit'].iloc[0]), "column": column},
                # {"name": "Plan", "value": str(catC['Sub Plan'].iloc[0]), "column": column},
                {"name": "Annual Limit", "value": str(catC['Annual Limit'].iloc[0]), "column": column},
                {"name": "Territories covered", "value": str(catC['Territory'].iloc[0]), "column": column},
                {"name": "Inpatient Co-Pay", "value": str(catC['IP Coinsurance'].iloc[0]), "column": column},
                {"name": "OP Consultation Deductible", "value": str(catC['PreExisting and Chronic Conditions'].iloc[0]), "column": column},
                {"name": "OP services co-pay", "value": str(catC['Diagnostic/Lab Copay'].iloc[0]), "column": column},
                {"name": "Prescribed Medicines - Limit", "value": str(catC['Limit of Phamacy'].iloc[0]), "column": column},
                {"name": "Prescribed Medicines - Coinsurance", "value": str(catC['Pharmacy'].iloc[0]), "column": column},
        ...
        fidelity_logger.debug("Category C Completed")

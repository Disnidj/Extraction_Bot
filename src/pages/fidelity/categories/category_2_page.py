...
class Category2Page:
    ...
    def get_field_selector(self, field_name, column=2):
    ...
    async def fill_category_2(self, catB, region, tpa, network):
        fidelity_logger.debug("Category B Started")
        print('Category B Started')

        try:
            await asyncio.sleep(10)
            await screenshot_and_compare(self.page, "fidelity", "FIDELITY_4")
        except Exception as e:
        ...
        if str(catB['Plan Selection'].iloc[0]).strip() == "NextcareEmployees Guard - Plan 1 - 5":
            fields = [
                ...
                {"name": "Medical Network", "value": str(catB['Pharmacy Sublimit'].iloc[0]), "column": column},
                {"name": "Room type", "value": str(catB['Room Category'].iloc[0]), "column": column},
                {"name": "Deductible per Consultation", "value": str(catB['Deductable'].iloc[0]), "column": column},
                {"name": "Prescribed Drugs & Medicines", "value": str(catB['Limit of Phamacy'].iloc[0]), "column": column},
                {"name": "Pharmacy Co-pay", "value": str(catB['Pharmacy'].iloc[0]), "column": column},
                {"name": "Diagnostics Co-pay", "value": str(catB['Diagnostic/Lab Copay'].iloc[0]), "column": column},
            ...
            fields = [
                # {"name": "Network", "value": str(catB['Pharmacy Sublimit'].iloc[0]), "column": column},
                # {"name": "Plan", "value": str(catB['Sub Plan'].iloc[0]), "column": column},
                {"name": "Annual Limit", "value": str(catB['Annual Limit'].iloc[0]), "column": column},
                {"name": "Territories covered", "value": str(catB['Territory'].iloc[0]), "column": column},
                {"name": "Inpatient Co-Pay", "value": str(catB['IP Coinsurance'].iloc[0]), "column": column},
                {"name": "OP Consultation Deductible", "value": str(catB['PreExisting and Chronic Conditions'].iloc[0]), "column": column},
                {"name": "OP services co-pay", "value": str(catB['Diagnostic/Lab Copay'].iloc[0]), "column": column},
                {"name": "Prescribed Medicines - Limit", "value": str(catB['Limit of Phamacy'].iloc[0]), "column": column},
                {"name": "Prescribed Medicines - Coinsurance", "value": str(catB['Pharmacy'].iloc[0]), "column": column},
        ...
        fidelity_logger.debug("Category B Completed")

...
from src.pages.fidelity.login_page import LoginPage
from src.pages.fidelity.quotation_page import QuotationPage
from src.pages.fidelity.process_page import ProcessPage
from src.pages.fidelity.categories.category_1_page import Category1Page
from src.pages.fidelity.categories.category_2_page import Category2Page
from src.pages.fidelity.categories.category_3_page import Category3Page
...
async def login_fidelity(playwright: async_playwright, referral_id):
    portal_name = 'Fidelity United'
    ...
    df1, df2 = read_excel('Fidelity United', referral_id)

    print("Fidelity login started")
    fidelity_logger.debug("Fidelity login started")

    # Check if the dataframes are empty
    if df1.empty:
        print("No data found sheet1")
        fidelity_logger.error("No data found sheet1")
        raise Exception("No data found sheet1")

    if df2.empty:
        print("Empty company data FIDELITY")
        # fidelity_logger.error("Empty company data FIDELITY")
        return
    ...
    if no_of_categories == 0:
        ...
        fidelity_logger.error("No categories found")
        raise Exception("No categories found")

    # Get the category 1 dataframe for each category
    cat1 = df2[df2['Category'] == unique_categories_list[0]]
    fidelity_logger.debug(f"Unique categories: {unique_categories_list}")
    ...
    process_page = ProcessPage(page,df1,df2,portal_name)
    await process_page.fill_process_form(df1, cat1,num_categories)

    region = str(df1[df1['KEY'] == "Emirates"]["VALUE"].values[0]).strip()
    tpa = str(df2['TPA'].values[0])

    # Fill the category 1 form
    category_1_page = Category1Page(page, portal_name)
    plan1 = str(cat1['Network'].values[0])
    await category_1_page.fill_category_1(cat1, region, tpa, plan1)

    # Fill the category 2 form if there are more than 1 category
    if no_of_categories > 1:
        category_2_page = Category2Page(page, portal_name)
        cat2 = df2[df2['Category'] == unique_categories_list[1]]
        plan2 = str(cat2['Network'].values[0])
        await category_2_page.fill_category_2(cat2, region, tpa, plan2)

    # Fill the category 3 form if there are more than 2 categories
    if no_of_categories > 2:
        category_3_page = Category3Page(page, portal_name)
        cat3 = df2[df2['Category'] == unique_categories_list[2]]
        plan3 = str(cat3['Network'].values[0])
        await category_3_page.fill_category_3(cat3, region, tpa, plan3)
    ...
    await context.close()
    await browser.close()
    fidelity_logger.debug("FIDELITY process completed")

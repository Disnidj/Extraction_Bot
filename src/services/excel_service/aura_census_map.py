...
def calculate_age(dob):
    ...
    print(f"Calculating age for DOB: {dob}, Today's date: {today}")
    print (f"Year difference: {today.year - dob.year}, Month and day comparison: {(today.month, today.day) < (dob.month, dob.day)}")
...
def aura_map_census_data(id):
    ...
    new_structure = pd.DataFrame({
        ...
        'Date of Birth (DD/MM/YY)': pd.to_datetime(sheet1_data['DOB'], dayfirst=True).dt.date,

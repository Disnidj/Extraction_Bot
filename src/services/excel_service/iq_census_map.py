    ...
    try:
        ...
        with xw.App(visible=False) as app:
            ...
            for index, row in merged_df.iterrows():
                # Convert DOB from d/m/y to m/d/y as datetime object
                dob_value = row['DOB']
                dob_dt = None
                try:
                    dob_dt = pd.to_datetime(dob_value, dayfirst=True, errors='coerce')
                except Exception:
                    pass
                cell = ws.range(f"B{index + 2}")
                if pd.notnull(dob_dt):
                    cell.value = dob_dt.to_pydatetime()
                    cell.number_format = 'm/d/yyyy'
                else:
                    cell.value = dob_value
